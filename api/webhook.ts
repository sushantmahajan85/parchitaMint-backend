...
/**
 * Mint an NFT based on the NFT ID from the memo
 * @param nftId The NFT ID from the memo
 * @param recipient The recipient address (usually the sender of the SOL)
...
async function mintNow(
  ...
  try {
    ...
    if (result.success) {
      ...
      console.error(`Failed to mint NFT with ID ${nftId}: ${result.error}`);
    }

    return result;
  } catch (error) {
    console.error(`Error minting NFT with ID ${nftId}:`, error);
    return {
      success: false,
      error: error instanceof Error ? error.message : String(error),
    };
